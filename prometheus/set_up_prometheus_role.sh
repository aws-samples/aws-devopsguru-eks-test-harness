#!/bin/bash -e
# Source: https://docs.aws.amazon.com/prometheus/latest/userguide/set-up-irsa.html#set-up-irsa-ingest

REGION=$(../get_region.sh)
CLUSTER_NAME=DevOpsGuruTestCluster
SERVICE_ACCOUNT_NAMESPACE=prometheus-server
AWS_ACCOUNT_ID=$(aws sts get-caller-identity --query "Account" --output text --region "$REGION")
OIDC_PROVIDER=$(aws eks describe-cluster --name $CLUSTER_NAME --query "cluster.identity.oidc.issuer" --output text --region "$REGION" | sed -e "s/^https:\/\///")
...
function getRoleArn() {
  OUTPUT=$(aws iam get-role --role-name $1 --query 'Role.Arn' --output text --region "$REGION" 2>&1)
...
if [ "$SERVICE_ACCOUNT_IAM_AMP_INGEST_ROLE_ARN" = "" ]; then
  #
